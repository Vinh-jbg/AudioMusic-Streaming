...
class Store:
    ...
    def add_music(self, name, image, path,artists):
        conn = Store.connect(self)
        cursor = conn.cursor()
        params = (name, image, path,artists)
        cursor.execute(
           "INSERT INTO tbl_radio (name, image, path, artists) VALUES (%s, %s, %s, %s);", (name, image, path, artists))
    ...
    def delete_music(self, id):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM tbl_radio WHERE id = %s;", (id,))
        conn.commit()
    ...
    def getAll(self):
        ...
        return cur.fetchall()
    # def getAll_artists(self):
    #     conn = Store.connect(self)
    #     cur = conn.cursor()
    #     data = cur.execute("SELECT DISTINCT artists FROM tbl_radio;")
    #     return cur.fetchall()
        # columns = [id]
        # for row in data:
        #     # print(row)
